...
Мы воспользовались *сопоставлением с образцом* (pattern matching). Конструкция `match points with` похожа на `switch` в таких языках, как C/C++/Java/C#, не гораздо богаче.

В случае, если мы получем пустой массив (образец `[]`), функция возвращает пустой список. Если список содержит один элемент (образец `[p]`), мы снова возрвращаем тот же самый список.

В других случаях мы берём точки списка попарно (функция `List.pairwise`), и оставляет только те пары, в которые дата и время второй точки больше, чем дата и время первой (функция `List.filter`). Наконец, мы выбрасываем первую точку из каждой пары. У нас остаётся список точек, начиная со второй, с правильной меткой времени.

Теперь все три теста у нас проходят.
...
Нетрудно заметить, что счётчик недель переполняется каждые 7168 дней (7&times;1024), или один раз в 19,62 лет. Апрель 2017-го&nbsp;&mdash; время очередного переполнения. Устройства, сделанные не совсем по стандарту, могут терять время.
