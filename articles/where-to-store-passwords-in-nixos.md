...
Для того, чтобы магия работала, важно расшифровывать настроечные файлы перед запуском приложений.
Например, в пайплайнах CI/CD.
Так что SOPS умеет интегрироваться с различными системами развёртывания, такими, как Ansible.

Удивительнее всего в SOPS то, что программа не умеет зашифровывать и расшифровывать данные.
Для этого её нужны внешние утилиты.

## Age

Одна из таких утилит — это Age, идущая на смену привычной рабочей лошадке PGP.
Она проще в использовании и больше подоходит для *Nix way*.

На самом деле нам не придётся работать с Age, посколько SOPS берёт все тяготы шифрования на себя.
...
Значит, нам потребуется ключ хоста, который традиционно хранят в каталоге **/etc/age** в файле **identity.key**:
...
Для пользователя с именем `eve` рекомендуют что-то вроде `user_eve`, а для хоста `webserver` — что-то вроде `host_webserver`.
