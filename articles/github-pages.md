...
как он будет представлен в адресной строке браузера.
...
<h2><a href="/service/2018/04/02/first-post/">Тестовая запись</a></h2>
<p>Проверка работы GitHub Pages.</p>

. . .
```

Посмотрим на то, какой код создал Jekyll. Дата **2018-04-02** (2 апреля 2018 года) была преобразована в часть пути, точно также, как и категория **service**.
Сама страница получила имя **first-post**. Полное имя нашей записи на сервере&nbsp;&mdash; **/service/2018/04/02/first-post/index.html**. Так как раньше мы
...
её адрес. А что такое `post.excerpt`? Это *анонс*. Обычно Jekyll использует в качестве анонса первый абзац записи. Мы можем изменить это поведение,
