...
> А ускорение запросов вряд ли удастся обнаружить статистически, потому что сравнение четырех или четырнадцати&nbsp;&mdash; только часть работы,
> которую выполняет SQL сервер. Очень небольшая часть.
>
> Поэтому я обойдусь без микрооптимизаций.

Лог может быть, например, таким:
...
Здесь СУБД просуммирует приблизительно треть нашей таблицы, но вернёт всего одно число. Для определённости мы будем считать, что объём выборки соответствует количеству обработанных записей, то есть в данном случае приблизительно равен трём миллионам.
