...
Для начала разберёмся, что такое быстрый поиск, и как он работает. Если вы изучали иностранный язык, то наверняка и заучивали слова. Один из 
...
Как видим, некоторые правила противоречат друг другу. С одной стороны, чем больше индексов, тем быстрее выполняются запросы и&nbsp;&mdash; в то же время&nbsp;&mdash; медленнее выполняются вставки. Ниже мы на конкретных примерах будем разбираться, как строить индексы правильно.

## Типичные задачи

Вооружившись библиотечной метафорой и знанием деталей, решим несколько поисковых задач.

### Найти книгу по автору и названию
...
SELECT *
  FROM Books
 WHERE Author = 'Лев Толстой' AND Title = 'Анна Каренина'
 FETCH FIRST 1 ROWS ONLY
```

Сразу несколько замечаний. Очевидно, таблицы в базе должны быть нормализованы. По меньше мере мы должны вынести в отдельную таблицу *авторов*, а также сами *произведения*. В приведённых примерах таблица `Books` денормализована. Я оставил её в таком некорректном виде потому, что хочу сконцентрироваться на индексах, оставив тему соеденения таблиц на потом.

Странная конструкция `FETCH FIRST 1 ROWS ONLY` соответствует `TOP 1` в MS SQL или `LIMIT 1 ` в My SQL. Она уже знакома тем, кто работает с DB2. Конструкция говорит серверу, что нам нужна только одна запись, а не все найденные.

#### Библиотека

Если в библиотеке есть индекс по именам и названиям, библиотекарь найдёт книги Льва Толстого, а среди них&nbsp;&mdash; любую &laquo;Анну Каренину&raquo;. Узнав из карточки, где находится книга, он отыщет её и принесёт.

То же самое случится, если в библиотеке есть индекс по названиям и именам, только библиотекарь сначала найдёт все книги с названием &laquo;Анна Каренина&raquo;.

Теперь представим, что у нас есть индекс только по имени автора. В реальной жизни так не бывает, но в учебном примере мы можем делать всё, что захотим. Итак, у нас в каждой карточке написано только имя автора и &laquo;адрес&raquo; книги на полках. Для того, чтобы найти Анну Каренину, нам надо взять все адреса и проверять книги в разных шкафах, пока мы не найдём нужную.

Поскольку шкафы могут находиться в разных метах библиотеке, поиск может сильно затянуться.

#### Сервер

Сервер в данном случае работает точно так же, как и библиотекарь. Есть у него есть индекс по имени и названию, либо по названию и имени, сервер им воспользуется.

Если индекс есть только по одному полю, скажем, по имени, сервер выберет диапазон книг в индексе, и затем будет подгружать страницы с данными, пытаясь перебором найти &laquo;Каренину&raquo;.
...
Если наша картотека упорядочена только по названию книги (по одному ключевому слову), сложно ли будет найти книгу Капрановой?

**Запрос**
```sql
SELECT *
  FROM Books
 WHERE Title = 'Книга о вкусной и здоровой пище'
   AND AuthorFamilyName = 'Капранова'
```

Оказывается, нет. Сначала мы ограничим поле поиска названием и получим несколько карточек, идущих подряд. Даже в очень большой библиотеке их окажется несколько десятков, не больше.
