...
Для начала давайте разберёмся, что такое быстрый поиск, и как он работает. Предположим, у нас есть телефонный справочник, где записаны все жители города с адресами и телефонами.
Чтобы сделать этот справочник, мы разолали по городу переписчиков с анкетами и составили один большой список. Жители в нашем списке находятся почти в случайном порядке.
Кто-то из переписчиков пришёл раньше, и все его жители оказались в справочнике первыми.

Предположим, мы хотим найти адрес Александра Сергеевича Пушкина. Нам придётся читать все записи подряд, пока мы не встретим нужное нам имя. Если справочник
содержит *N* жителей, то в худшем случае нам придётся прочитать *N* записей.
...
Третья колонка таблицы показывает отношение N и &lceil;log₂ N&rceil;. Для тысячи чисел оно равно 100, то есть двоичный поиск работает в сто раз быстрее, чем перебор.
Для миллиона чисел двоичный поиск работает в пятьдесят тысяч раз быстрее, а для миллиарда&nbsp;&mdash; в тридцать миллионов раз!
...
Давайте теперь предположим, что мы хотим найти телефон человека, зная его адрес. У нас есть телефонный справочник, где записи упорядочены по фамилии. Сколько
времени нам потребуется, чтобы завершить поиск? Оказывается, что в худшем случае речь снова идёт о *N* попытках. То, что справочник упорядочен по фамилии
никак не помогает искать человека по адресу.
